...
if (isset($_POST['add_course'])) {
    $new_id = count($courses) ? max(array_column($courses, 'id')) + 1 : 1;
    $image = "";
...
if ($delete_id) {
    foreach ($courses as $key => $course) {
        if ($course['id'] == $delete_id) {
            unset($courses[$key]);
            $_SESSION['courses'] = array_values($courses); // reindex array
            header("Location: courses.php");
            exit();
        }
    }
...
<style>
    .table_th { padding: 15px; background: #ddd; }
    .table_td { padding: 10px; text-align: center; background: #f9f0dc; }
